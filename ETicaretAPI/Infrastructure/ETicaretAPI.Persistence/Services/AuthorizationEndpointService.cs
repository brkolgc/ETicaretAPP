...
namespace ETicaretAPI.Persistence.Services
{
    public class AuthorizationEndpointService : IAuthorizationEndpointService
    {
        ...
        public async Task AssignRoleEndpointAsync(string[] roles, string menu, string code, Assembly assembly)
        {
            Menu _menu = await _menuReadRepository.GetSingleAsync(m => m.Name == menu);

            if (_menu == null)
            {
                _menu = new Menu { Id = Guid.NewGuid(), Name = menu };
            ...
            if (endpoint == null)
            {
                var action = _applicationService.GetAuthorizeDefinitionEndpoints(assembly)
